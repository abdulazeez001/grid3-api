...
const tracingMiddleware = (req, res, next, controllerMethod) => {
  // Initialize the Tracer
  opentracing.initGlobalTracer(tracer);
  const wireCtx = tracer.extract(opentracing.FORMAT_HTTP_HEADERS, req.headers);
  // Creating our span with context from incoming request
  const span = tracer.startSpan(`${req.baseUrl}${req.route.path}`, {
    childOf: wireCtx,
  });
  ...
  span.setTag(opentracing.Tags.HTTP_URL, `${req.baseUrl}${req.path}`);
  span.setTag("email", `${req.user && req.user.email}`);
  ...
  // return the execution of the controller method
